...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImRect check_bb(pos, pos + ImVec2(square_sz * 0.625f, square_sz * 0.625f));
    ...
    if (mixed_value)
    {
        ...
        window->DrawList->AddRectFilled(check_bb.Min * 0.8f + pad / 4.f, check_bb.Max * 0.8f - pad / 4.f, check_col, style.FrameRounding);
    }
    else if (*v)
    {
        const float pad = ImMax(1.0f, IM_FLOOR(square_sz / 6.0f));
        RenderCheckMark(window->DrawList, check_bb.Min + ImVec2(pad, pad), check_col, square_sz * 0.625f - pad * 2.0f);
    }

    ImVec2 label_pos = ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x, check_bb.Min.y - 3.f);
    if (g.LogEnabled)
